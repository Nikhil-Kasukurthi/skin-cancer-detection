...
def move_image(type_, dataframe, directory):
	...
	for index, image in enumerate(dataframe['image_id']):
		...
		#print(train_df.loc[train_df['image_id']==image])
		#print(train_df.loc[train_df['image_id']==image]['melanoma'])

		if dataframe.loc[dataframe['image_id']==image]['melanoma'].all() == 1:
			os.rename(image_path, os.path.join(type_,'melanoma',image+'.jpg'))
			if(type_=='train'):
				labels_train[index][0]=1
			else:
				labels_validation[index][0]=1
		elif dataframe.loc[dataframe['image_id']==image]['seborrheic_keratosis'].all() == 1:
			os.rename(image_path, os.path.join(type_,'seborrheic_keratosis',image+'.jpg'))
			if(type_=='train'):
...
pickle.dump('labels.pkl', [labels_train, labels_validation])